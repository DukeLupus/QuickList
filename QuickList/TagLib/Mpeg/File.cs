...
namespace Sander.QuickList.TagLib.Mpeg
{
	...
	/// </summary>
	/// <remarks>
	///     A <see cref="TagLib.Id3v1.Tag" /> and
	///     <see
	///         cref="TagLib.Id3v2.Tag" />
	///     will be added automatically to any
	...
	public sealed class File : NonContainer.File
	{
		...
		internal void ReadSystemFile(long position)
		{
			...
			for (var i = 0;
				i < sanity_limit && (start_time == null ||
				                     !audio_found || !video_found);
				i++)
			{
				switch (FindMarker(ref position))
				{
		...
		private void ReadSystemSyncPacket(ref long position)
		{
			int packet_size;
			Seek(position + 4);
			var version_info = ReadBlock(1)[0];
