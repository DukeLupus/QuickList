...
namespace Sander.QuickList.TagLib
{
	...
	public class ByteVector : IList<byte>, IComparable<ByteVector>
	{
		...
		public int EndsWithPartialMatch(ByteVector pattern)
		{
			...
			var start_index = data.Count - pattern.Count;
		...
		public string ToString(StringType type, int offset, int count)
		{
			...
			var s = StringTypeToEncoding(type, bom)
			...
			bool copyFirstChunk)
		{
			...
			while (true)
			{
				...
				var n = stream.Read(bytes, 0, read_size);
				vector.Add(bytes);
				bytes_read += n;
